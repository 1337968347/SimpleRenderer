...
vec4 getNoise(vec2 uv) {
  vec2 uv0 = (uv / 103.0) + vec2(time / 17.0, time / 29.0) * 0.3;
  vec2 uv1 = uv / 107.0 - vec2(time / -19.0, time / 31.0) * 0.3;
  vec2 uv2 = uv / vec2(897.0, 983.0) + vec2(time / 101.0, time / 97.0) * 0.3;
  vec2 uv3 = uv / vec2(991.0, 877.0) - vec2(time / 109.0, time / -113.0) * 0.3;
